...
**whisper_api** (port 8000) — faster-whisper wrapper exposing OpenAI-compatible `POST /v1/audio/transcriptions`. `engine.py` loads the model once and serializes access with a lock; PyAV decode errors are mapped to `InvalidAudioError`. `faster-whisper` is deliberately NOT in the `dev` extra (heavy) — `engine.py` imports it lazily, so fast tests run without it.
